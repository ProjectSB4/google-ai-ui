...
def respond():
    ...
    if modelchoose.get() == "Gemini Pro":
        ...
        if not (img := Path("tmp/image.png")).exists():
            error("PLEASE MAKE SURE THE IMAGE IS NAMED image.png and that it exists")
        image_parts = [
            {
                "mime_type": "image/png",
...
def answerboxfill(fill):
    ...
    answer_box.configure(state="readonly")
def error(message):
    main.destroy()
...
se = ctk.CTkButton(main, text="Edit Safety Settings", fg_color="#b55100", hover_color="#b50000", command=viewsafteyins)
...
answer_box = ctk.CTkEntry(main, 590, 50, state="readonly")
